...
// Fix the corrupted forEach loop by adding the missing input field creation code
content = content.replace(
    /STATE\.selectedMethods\.forEach\(\(method, index\) => \{\s+const isReadOnly = STATE\.selectedMethods\.length === 1;\s+\s+\}\)/,
    `STATE.selectedMethods.forEach((method, index) => {
